...
# 生成的grpc代码存放目录 (重构后路径)
GRPC_OUT_DIR = os.path.join(PROJECT_ROOT, "app", "grpc")
...
def install_grpc_tools():
    ...
    try:
        subprocess.run(
            [python_exec, "-m", "pip", "install", "grpcio", "grpcio-tools"], check=True
        )
            ...
            if file.endswith("_pb2_grpc.py"):
                ...
                try:
                    ...
                    for i, line in enumerate(lines):
                        ...
                        if match:
                            pb2_module = match.group(1)
                            alias = match.group(2)
                            lines[i] = f'from . import {pb2_module} as {alias}'
                            modified = True
                            print(f"修复导入: {line} -> {lines[i]}")
                    
                    # 替换子目录中的导入路径
                    # 例如: from auth import CurrentUser_pb2 -> from . import CurrentUser_pb2
                    for i, line in enumerate(lines):
                        if line.startswith("from auth import"):
                            lines[i] = line.replace("from auth import", "from . import")
                            modified = True
                            print(f"修复导入: {line} -> {lines[i]}")
