...
class DongcaiKLineSource(BaseSource):
    ...
    def _get_zh_data_on_time(self, code: str) -> Optional[KLine]:
        ...
        for i in range(len(tickers)):
            ...
            if ticker_code.startswith("0") or ticker_code.startswith("3"):
                ...
                ticker_code.startswith("6")
                or ticker_code.startswith("7")
                or ticker_code.startswith("9")
            ):
    ...
    def get_kl(
        ...
        start = datetime.datetime.strptime(start_date, "%Y-%m-%d").strftime("%Y%m%d")
            ...
            datetime.datetime.strptime(end_date, "%Y-%m-%d").strftime("%Y%m%d")
        ...
        if code.startswith("US"):
            data = ak.stock_us_hist(
                symbol=f"105.{code[3:]}", start_date=start, end_date=end, adjust="qfq"
            )
        elif code.startswith("HK"):
            data = ak.stock_hk_hist(
                symbol=code[3:], start_date=start, end_date=end, adjust="qfq"
            )
        elif code.startswith("SZ") or code.startswith("SH"):
        ...
        if data is not None:
            return [
                KLine(
                    time_key=data["日期"][index].strftime("%Y-%m-%d")
                    if isinstance(data["日期"][index], datetime.date)
                    else data["日期"][index],
                    high=data["最高"][index],
                    low=data["最低"][index],
                    open=data["开盘"][index],
