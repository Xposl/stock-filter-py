...
class NewsAggregatorManager:
    ...
    async def fetch_all_active_sources(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            active_sources = await self.news_source_repo.get_news_sources_by_status(NewsSourceStatus.ACTIVE)
    ...
    async def _save_articles_to_db(self, articles: List[Dict[str, Any]]):
        ...
        for article_data in articles:
            try:
                ...
                if url_hash:
                    existing = await self.news_article_repo.get_article_by_url_hash(url_hash)
                    if existing:
                ...
                article = await self.news_article_repo.create_news_article(article_create)
                if article:
                    saved_count += 1
