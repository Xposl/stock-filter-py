...
class RSSAggregator:
    ...
    async def __aenter__(self):
        ...
        if self._own_session:
            ...
            connector = aiohttp.TCPConnector(
                limit=100, limit_per_host=30, ssl=ssl_context  # 使用宽松的SSL配置
            )
    ...
    async def _parse_rss_entry(
        self, entry: Any, news_source: NewsSource
    ...
    def _should_include_article(
        ...
        if news_source.filter_keywords:
            ...
            # 如果没有包含任何关键词，则过滤掉
            has_keyword = any(
                keyword.lower() in title_and_summary
        ...
        if news_source.filter_categories:
            ...
            if article_category:
                has_category = any(
                    category.lower() in article_category
