...
class APIAggregator:
    ...
    async def _fetch_xueqiu_feed(self, news_source: NewsSource) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "热门话题" in news_source.name:
                # 热门话题API
                params = {
                    'size': news_source.max_articles_per_fetch or 20,
                    'page': 1
                }
                url = news_source.url
    ...
    async def _fetch_eastmoney_feed(self, news_source: NewsSource) -> List[Dict[str, Any]]:
        """
        抓取东方财富API数据
    ...
    async def _fetch_generic_api_feed(self, news_source: NewsSource) -> List[Dict[str, Any]]:
        """
        通用API抓取
        ...
        try:
            ...
            # 通用参数
            params = {
                'limit': news_source.max_articles_per_fetch or 20,
                'page': 1
            }
            ...
            async with self.session.get(news_source.url, params=params) as response:
                ...
                if isinstance(data, dict):
                    # 尝试常见的数据字段
                    items = data.get('data', data.get('items', data.get('list', data.get('articles', []))))
                else:
                    items = data if isinstance(data, list) else []
    ...
    def _parse_xueqiu_item(self, item: Dict[str, Any], news_source: NewsSource) -> Optional[Dict[str, Any]]:
        """解析雪球条目"""
        try:
            # 提取基础信息
            ...
            if isinstance(created_at, (int, float)):
                published_at = datetime.fromtimestamp(created_at / 1000 if created_at > 1e10 else created_at, tz=timezone.utc)
            else:
            ...
            # 简单的重要性算法
            importance_score = min(1.0, (retweet_count * 0.3 + reply_count * 0.4 + fav_count * 0.3) / 100)
    ...
    def _parse_eastmoney_item(self, item: Dict[str, Any], news_source: NewsSource) -> Optional[Dict[str, Any]]:
        """解析东方财富条目"""
        try:
            # 提取基础信息
            ...
            # 时间处理
            pub_time = item.get('pub_time', item.get('REPORT_DATE', item.get('create_time', '')))
            if pub_time:
                try:
    ...
    def _parse_generic_item(self, item: Dict[str, Any], news_source: NewsSource) -> Optional[Dict[str, Any]]:
        """解析通用API条目"""
        try:
            # 尝试多种可能的字段名
                ...
                if field in item and item[field]:
                    try:
                        ...
                        if isinstance(time_val, str):
                            published_at = datetime.fromisoformat(time_val.replace('Z', '+00:00'))
                        elif isinstance(time_val, (int, float)):
                            published_at = datetime.fromtimestamp(time_val, tz=timezone.utc)
                        break
                    except:
                        continue
