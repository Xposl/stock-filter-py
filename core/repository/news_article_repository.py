...
from ..models.news_article import (
    ...
    NewsArticleCreate,
    NewsArticleUpdate,
    dict_to_news_article,
    news_article_to_dict,
)
...
class NewsArticleRepository:
    ...
    async def get_articles_by_source_id(
        ...
        try:
            ...
            results = self.db.query(
                sql, {"source_id": source_id, "limit": limit, "offset": offset}
            )
    ...
    async def get_articles_by_status(
        self, status: ArticleStatus, limit: int = 100, offset: int = 0
    ) -> list[NewsArticle]:
        """
        根据状态获取文章列表
        ...
        try:
            ...
            """
            results = self.db.query(
                sql, {"status": status.value, "limit": limit, "offset": offset}
            )
    ...
    async def get_recent_articles(
        ...
        try:
            ...
            LIMIT :limit
            """
            results = self.db.query(sql, {"cutoff_time": cutoff_time, "limit": limit})
    ...
    async def search_articles(
        ...
        try:
            ...
            """
            results = self.db.query(
                sql, {"query": search_term, "limit": limit, "offset": offset}
            )
        ...
        try:
            ...
            if search:
                ...
                where_clauses.append(
                    "(title LIKE :search OR content LIKE :search OR summary LIKE :search)"
                )
    ...
    async def update_news_article(
        self, article_id: int, update_data: NewsArticleUpdate
    ) -> Optional[NewsArticle]:
        """
        更新新闻文章
    ...
    async def update_article_processing_status(
        self, article_id: int, status: ArticleStatus, error_message: str = None
    ) -> bool:
        """
        更新文章处理状态
    ...
    async def batch_create_articles(
        self, articles: list[NewsArticleCreate]
    ) -> list[NewsArticle]:
        """
        批量创建新闻文章
    ...
    async def get_article_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            stats["by_source"] = (
                {row["source_name"]: row["count"] for row in source_results}
                if source_results
                else {}
            )
