...
class VolumeSuperTrendAIStrategy(BaseStrategy):
    ...
    def set_params(self, param):
        ...
        self.KNN_PriceLen = (
            param["KNN_PriceLen"] if "KNN_PriceLen" in param else self.KNN_PriceLen
        )
        self.KNN_STLen = param["KNN_STLen"] if "KNN_STLen" in param else self.KNN_STLen
        self.volume_filter = (
            param["volume_filter"] if "volume_filter" in param else self.volume_filter
        )
        self.volume_threshold = (
            param["volume_threshold"]
            if "volume_threshold" in param
            else self.volume_threshold
        )
        self.confirm_days = (
            param["confirm_days"] if "confirm_days" in param else self.confirm_days
        )
        self.trend_filter = (
            param["trend_filter"] if "trend_filter" in param else self.trend_filter
        )
    ...
    def calculate(self, kl_data: list[KLine]):
        ...
        if (
            length
            < max(self.len, self.KNN_PriceLen, self.KNN_STLen, self.ma_period) + self.n
        ):
            # 数据不足
            return [0] * length
        ...
        # 计算成交量均值用于过滤
        utils = UtilsHelper()
        utils.sma(volumeData, 20)

        # 计算长期趋势用于过滤
        if self.trend_filter:
            utils.ema(closeData, self.ma_period)
    ...
    def _detect_ai_signal(self, indicator_pos_data, i):
        ...
        if indicator_pos_data[i] == 1 and (indicator_pos_data[i - 1] != 1):
            return True, 1
