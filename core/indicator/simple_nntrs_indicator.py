...
class SimpleNNTRSIIndicator:
    ...
    def __init__(self, rsi_length=14, nn_length=5, sd_lookback=365, sd_mult=2.0, use_median=False):
    ...
    def getKey(self):
        """获取指标的唯一键名"""
        return f'SimpleNNTRSI_indicator'
    ...
    def _apply_nn_smoothing(self, rsiData):
        """应用简单神经网络平滑
        ...
        for i in range(length):
            ...
            for j in range(lookback):
                # 每个值乘以相同权重
                sum_value += rsiData[i - j] * (1.0 / self.nn_length)
            
            nnOutput.append(sum_value)
    ...
    def calculate(self, klData):
        ...
        # 计算RSI
        rsiData = self._calculate_rsi(klData)
        
        # 应用神经网络平滑
        nnOutput = self._apply_nn_smoothing(rsiData)
        
        # 计算标准差带
        center, upper1, lower1, upper2, lower2 = self._calculate_std_bands(nnOutput)
        
        # 生成买卖信号 (对应"Trend Following"模式)
        posData = []
        for i in range(length):
            if i < 1:
                posData.append(0)  # 第一个数据点没有信号
                continue
                
            # 趋势跟随信号
            # 根据ThinkorSwim版本：col = if (if NoHA then nnOutput else harc) > cross then 1 else -1
            cross_value = 50 if not self.use_median else center[i]
            
            if nnOutput[i] > cross_value:
                # 当NN-RSI高于中线/中值线时做多
                posData.append(1)
            else:
                # 当NN-RSI低于中线/中值线时做空
