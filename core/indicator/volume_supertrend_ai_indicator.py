...
class VolumeSuperTrendAIIndicator:
    ...
    def calculate(self, klData):
        ...
        # 计算super_trend
        posData = []
        super_trend, direction = self._calculate_super_trend(close_data, highData, lowData, volume_data)
    ...
    def _calculate_super_trend(self, close_data, highData, lowData, volume_data):
        ...
        for i in range(1, length):
            lower_band[i] = lower_band[i] if lower_band[i] > lower_band[i-1] or close_data[i-1] < lower_band[i-1] else lower_band[i-1]
            upper_band[i] = upper_band[i] if upper_band[i] < upper_band[i-1] or close_data[i-1] > upper_band[i-1] else upper_band[i-1]
