...
class TickerStrategyBase(BaseModel):
    ...
    data: Optional[Union[list[Any], dict[str, Any]]] = Field(
        default=None, description="策略数据"
    )
    pos_data: Optional[Union[list[Any], dict[str, Any]]] = Field(
        default=None, description="持仓数据"
    )
...
# 用于序列化和反序列化的辅助函数
def ticker_strategy_to_dict(
    strategy: Union[TickerStrategy, TickerStrategyCreate, TickerStrategyUpdate]
) -> dict:
    """
