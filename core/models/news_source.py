...
# 用于序列化和反序列化的辅助函数
def news_source_to_dict(source: Union[NewsSource, NewsSourceCreate, NewsSourceUpdate]) -> dict:
    """
    ...
    if 'filter_keywords' in result and result['filter_keywords'] is not None:
        result['filter_keywords'] = json.dumps(result['filter_keywords'], ensure_ascii=False)
    
    if 'filter_categories' in result and result['filter_categories'] is not None:
        result['filter_categories'] = json.dumps(result['filter_categories'], ensure_ascii=False)
        ...
        if json_field in processed_data:
            ...
            # 尝试解析JSON字符串
                ...
                try:
                    ...
                    processed_data[json_field] = parsed if isinstance(parsed, list) else None
                except (ValueError, json.JSONDecodeError):
                    processed_data[json_field] = None
    
    # 确保必要字段存在并有默认值
    processed_data['update_frequency'] = int(processed_data.get('update_frequency', 3600) or 3600)
    processed_data['max_articles_per_fetch'] = int(processed_data.get('max_articles_per_fetch', 50) or 50)
    processed_data['total_articles_fetched'] = int(processed_data.get('total_articles_fetched', 0) or 0)
