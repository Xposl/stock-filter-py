...
class TickerStrategyRepository:
    ...
    def __init__(self, db_connection: Optional[Any] = None):
        ...
        # 策略服务
        self.strategy = Strategy(self.db)
    ...
    def get_item_by_ticker_id_and_strategy_id(self, ticker_id: int, strategy_id: int, kl_type: str) -> Optional[TickerStrategyModel]:
        """根据股票ID, 策略ID和K线类型获取策略记录
        ...
        """
        try:
            sql = f"SELECT * FROM {self.table} WHERE ticker_id = ? AND strategy_id = ? AND kl_type = ?"
            result = self.db.query_one(sql, (ticker_id, strategy_id, kl_type))
            return dict_to_ticker_strategy(result) if result else None
        except Exception as e:
            logger.error(f"获取策略记录错误: {e}")
            return None

    def update_item(self, ticker_id: int, strategy_key: str, kl_type: str, time_key: str, entity: Dict[str, Any]) -> None:
        """更新或创建策略记录
        ...
        try:
            # 获取策略信息
            strategy = self.strategy.getItemByKey(strategy_key)
            strategy_id = strategy['id']

            # 检查是否存在该记录
            existing_item = self.get_item_by_ticker_id_and_strategy_id(ticker_id, strategy_id, kl_type)
            ...
            if existing_item is None:
                ...
                # 添加条件参数
                values.extend([ticker_id, strategy_id, kl_type])
                
                # 构建SQL
                sql = f"UPDATE {self.table} SET {', '.join(set_clauses)} WHERE ticker_id = ? AND strategy_id = ? AND kl_type = ?"
