...
class InvestmentOpportunityNode(AsyncNode):
    ...
    def _identify_investment_opportunity(
        self, article: dict, sentiment: dict
    ) -> dict[str, Any]:
        """识别单个投资机会"""
    ...
    def _calculate_opportunity_score(
        self, sentiment: dict, opp_types: list, industries: list
    ) -> float:
        """计算投资机会评分"""
...
class RiskAssessmentNode(AsyncNode):
    ...
    async def run_async(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for article, opportunity, sentiment in zip(
            articles, opportunities, sentiment_results
        ):
            risk_assessment = self._assess_investment_risk(
                article, opportunity, sentiment
            )
    ...
    def _assess_investment_risk(
        self, article: dict, opportunity: dict, sentiment: dict
    ) -> dict[str, Any]:
        """评估单项投资风险"""
        title = article.get("clean_title", "")
        content = article.get("clean_content", "")
    ...
    def _calculate_risk_score(
        self, risk_factors: dict, sentiment: dict, opportunity: dict
    ) -> float:
        """计算风险评分"""
        base_risk = 5.0  # 基础风险
    ...
    def _calculate_overall_risk(self, risk_assessments: list[dict]) -> dict[str, Any]:
        ...
        avg_risk_score = sum(r["risk_score"] for r in risk_assessments) / len(
            risk_assessments
        )
        ...
        return {
            ...
            "high_risk_count": len(
                [r for r in risk_assessments if r["risk_score"] >= 7]
            ),
        }
...
class StockCorrelationNode(AsyncNode):
    ...
    def _analyze_stock_correlation(
        self, article: dict, opportunity: dict
    ) -> dict[str, Any]:
        """分析股票关联度"""
        # 简化实现 - 实际应该结合股票数据库
        companies = opportunity.get("key_information", {}).get(
            "mentioned_companies", []
        )
...
class InvestmentAdviceGenerationNode(AsyncNode):
    ...
    async def run_async(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for _i, (opp, risk, correlation, sentiment) in enumerate(
        ...
        return {
            ...
            "actionable_recommendations": self._filter_actionable_advice(
                investment_advice
            ),
    ...
    def _generate_investment_advice(
        self, opportunity: dict, risk: dict, correlation: dict, sentiment: dict
    ) -> dict[str, Any]:
        """生成单项投资建议"""
        ...
        return {
            ...
            "rationale": rationale,
            "confidence": self._calculate_advice_confidence(
                opportunity, risk, sentiment
            ),
    ...
    def _determine_investment_action(
        self, opportunity: dict, risk: dict, sentiment: dict
    ) -> str:
        """确定投资动作"""
    ...
    def _generate_overall_strategy(self, advice_list: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "actionable_count": len(
                [a for a in advice_list if a["investment_action"] in ["买入", "强烈买入"]]
            ),
        }
...
class InvestmentAnalysisFlow:
    ...
    async def analyze_investment(
        self, articles_with_sentiment: dict[str, Any]
    ) -> dict[str, Any]:
        """
        执行投资分析
