...
class TextPreprocessingNode(AsyncNode):
    ...
    def _preprocess_article(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 计算文本长度
        processed["text_length"] = len(processed["clean_title"]) + len(processed["clean_content"])
...
class KeywordSentimentNode(AsyncNode):
    ...
    def _extract_phrase_around_word(self, text: str, word: str, context_length: int = 20) -> str:
        """提取词汇周围的短语"""
        word_index = text.find(word)
        if word_index == -1:
            return ""
...
class ContextualSentimentNode(AsyncNode):
    ...
    async def run_async(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.llm_client:
            # 使用简化的启发式方法
            contextual_results = await self._heuristic_contextual_analysis(articles, keyword_results)
        else:
            # 使用LLM进行深度分析
            contextual_results = await self._llm_contextual_analysis(articles, keyword_results)
    ...
    def _analyze_context_factors(self, article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 基于因素调整置信度
        factor_count = sum(1 for v in factors.values() if isinstance(v, dict) and v.get("detected", False))
    ...
    def _adjust_score_by_context(self, base_score: float, context_factors: Dict) -> float:
        """根据上下文因素调整情感评分"""
...
class SentimentIntegrationNode(AsyncNode):
    ...
    async def run_async(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, (keyword_result, contextual_result) in enumerate(zip(keyword_results, contextual_results)):
            integrated_result = self._integrate_sentiment_scores(keyword_result, contextual_result)
    ...
    def _integrate_sentiment_scores(
        ...
        # 计算综合置信度
        final_confidence = min(1.0, (keyword_confidence + contextual_confidence) / 2 + 0.1)
    ...
    def _calculate_overall_sentiment(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        if total_weight > 0:
            weighted_score = sum(
                r["final_sentiment_score"] * r["confidence"] 
                for r in results
            ) / total_weight
        else:
            weighted_score = sum(r["final_sentiment_score"] for r in results) / len(results)
        
        # 统计情感分布
        sentiment_distribution = {
            "positive": len([r for r in results if r["final_sentiment_score"] > 1]),
            "neutral": len([r for r in results if -1 <= r["final_sentiment_score"] <= 1]),
    ...
    def _generate_sentiment_summary(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        # 找到最强烈的情感
        strongest_sentiment = max(results, key=lambda x: abs(x["final_sentiment_score"]))
