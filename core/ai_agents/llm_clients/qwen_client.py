...
import logging
import asyncio
from typing import Dict, List, Optional, Union, AsyncGenerator, Generator, Any
...
try:
    ...
    raise ImportError(f"需要安装 openai 库: pip install openai>=1.0.0") from e
...
class QwenClient:
    ...
    def _handle_stream_response(self, response) -> Generator[QwenStreamResponse, None, None]:
        """处理流式响应"""
        for chunk in response:
            yield QwenStreamResponse(
                ...
                model=chunk.model,
                choices=[choice.model_dump() for choice in chunk.choices] if chunk.choices else [],
...
class AsyncQwenClient:
    ...
    async def chat_completions_create(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"发送千问异步API请求: model={request_params['model']}, stream={stream}")
    ...
    async def _handle_async_stream_response(self, response) -> AsyncGenerator[QwenStreamResponse, None]:
        """处理异步流式响应"""
...
def stream_chat(prompt: str, system_prompt: Optional[str] = None, **kwargs) -> Generator[str, None, None]:
    """
    便捷的流式对话函数
...
async def async_stream_chat(prompt: str, system_prompt: Optional[str] = None, **kwargs) -> AsyncGenerator[str, None]:
    """
    便捷的异步流式对话函数
