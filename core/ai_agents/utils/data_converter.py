...
class NewsDataConverter:
    ...
    def _extract_datetime(self, data: Dict, field_names: List[str]) -> Optional[datetime]:
        """提取日期时间字段"""
        for field_name in field_names:
            if field_name in data and data[field_name]:
                value = data[field_name]
    ...
    def _extract_tags(self, data: Dict) -> List[str]:
        ...
        # 去重并过滤
        return list(set([tag for tag in tags if tag and len(tag) > 1]))
...
class AnalysisResultConverter:
    ...
    def convert_sentiment_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "dominant_emotion": result.get("sentiment_breakdown", {}).get("dominant_emotion", "neutral"),
            "key_phrases": result.get("key_indicators", {}).get("sentiment_phrases", []),
            "analysis_details": {
...
class APIResponseConverter:
    ...
    def convert_batch_analysis_response(
        self, 
        results: List[StandardAnalysisResult],
        ...
        return {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "average_processing_time": total_processing_time / max(1, len(results)),
                    "timestamp": datetime.now(timezone.utc).isoformat()
                }
            }
        }
    
    def _generate_batch_summary(self, results: List[StandardAnalysisResult]) -> Dict[str, Any]:
        """生成批量分析摘要"""
        if not results:
            return {}
        ...
        return {
            ...
            "investment_overview": {
                ...
                "total_opportunities": sum(len(r.investment_opportunities) for r in results),
                "actionable_count": len([r for r in results if any("买入" in rec.get("action", "") for rec in r.recommendations)])
            },
            "confidence_metrics": {
...
def convert_news_data(
    ...
    if isinstance(raw_data, list):
        ...
        return converter.convert_to_standard(raw_data, source_type, source_name, source_id)
