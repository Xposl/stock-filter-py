...
from .xueqiu_news_client import XueqiuNewsClient
...
class XueqiuClientFactory:
    ...
    def create_client(self, client_type: Union[XueqiuClientType, str], 
                     session: Optional[aiohttp.ClientSession] = None) -> XueqiuBaseClient:
        """
        创建雪球客户端
        ...
        if client_type == XueqiuClientType.NEWS:
            client = XueqiuNewsClient(session=client_session)
    ...
    def get_news_client(self, session: Optional[aiohttp.ClientSession] = None) -> XueqiuNewsClient:
        """
        获取新闻客户端
        ...
        Returns:
            雪球新闻客户端
        """
        return self.create_client(XueqiuClientType.NEWS, session)
    
    def get_stock_client(self, session: Optional[aiohttp.ClientSession] = None) -> XueqiuStockClient:
        """
        获取股票客户端
    ...
    async def create_async_client(self, client_type: Union[XueqiuClientType, str], 
                                 auto_session: bool = True) -> XueqiuBaseClient:
        """
        创建异步雪球客户端
    ...
    def configure_client(self, client_type: Union[XueqiuClientType, str], 
    ...
    def get_client_config(self, client_type: Union[XueqiuClientType, str]) -> Dict[str, Any]:
        """
        获取客户端配置
    ...
    async def cleanup(self):
        ...
        try:
            ...
            for client in self._clients.values():
                if hasattr(client, 'session') and client.session and client._own_session:
...
def create_news_client(session: Optional[aiohttp.ClientSession] = None) -> XueqiuNewsClient:
    """
    快速创建新闻客户端
    ...
    return factory.get_news_client(session)


def create_stock_client(session: Optional[aiohttp.ClientSession] = None) -> XueqiuStockClient:
    """
    快速创建股票客户端
