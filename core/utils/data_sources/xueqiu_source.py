...
from .base_source import KLine, BaseSource
        ...
        if ticker is not None:
            ...
            if quote.quote.status in self.status:
                if quote.quote.status == 0 or quote.quote.status == 3:
                    ticker.is_deleted = True
                ticker.status = self.status[quote.quote.status] if quote.quote.open is not None else 0
            else:
            ...
            ticker.lot_size = quote.quote.lot_size if quote.quote.lot_size is not None else -1
            ticker.pe_forecast = quote.quote.pe_forecast if quote.quote.pe_forecast is not None else -1
            ...
            ticker.total_share = quote.quote.total_shares if quote.quote.total_shares is not None else -1
        return ticker