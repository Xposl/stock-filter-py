...
class AuthGrpcClient:
    ...
    def _initialize_connection(self):
        ...
        try:
            ...
            self.channel = grpc.insecure_channel(
                f"{AUTH_SERVICE_HOST}:{AUTH_SERVICE_PORT}", options=options
            )
    ...
    def get_current_user(self, token: str) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            response = stub.queryCurrentUser(request, metadata=metadata, timeout=10.0)
            ...
            if e.code() == grpc.StatusCode.UNAVAILABLE:
                logger.error(
                    f"无法连接到认证服务 {AUTH_SERVICE_HOST}:{AUTH_SERVICE_PORT}，请检查服务是否运行"
                )
