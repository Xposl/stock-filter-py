...
class AKShareIndustryProvider(StockDataProvider):
    ...
    async def get_all_industry_boards(self) -> pd.DataFrame:
        ...
        try:
            ...
            if (
                ...
                and (now - self._cache_timestamp).seconds < self._cache_ttl
            ):
            ...
            board_df = await loop.run_in_executor(
                self.executor, ak.stock_board_industry_name_em
            )
    ...
    async def _find_matching_boards(self, industry: str) -> list[str]:
        ...
        try:
            ...
            # 2. 包含匹配
            if not matched_boards:
                contains_matches = board_df[
                    board_df["板块名称"].str.contains(industry, na=False)
                ]
            ...
            if not matched_boards and industry in self.industry_keywords:
                ...
                for keyword in keywords:
                    keyword_matches = board_df[
                        board_df["板块名称"].str.contains(keyword, na=False)
                    ]
    ...
    async def _get_board_constituent_stocks(self, board_name: str) -> list[dict]:
        ...
        try:
            ...
            constituent_df = await loop.run_in_executor(
                self.executor, ak.stock_board_industry_cons_em, board_name
            )
    ...
    def _calculate_relevance_score(self, row: dict) -> int:
        ...
        try:
            ...
            # 价格稳定性加分 (涨跌幅适中)
            change_pct = (
                abs(float(row.get("涨跌幅", 0))) if pd.notna(row.get("涨跌幅")) else 0
            )
    ...
    def _deduplicate_and_rank_stocks(
        self, stocks: list[dict], impact_degree: int
    ) -> list[dict]:
        """去重并按相关性排序股票"""
        # 按股票代码去重，保留评分最高的
        unique_stocks = {}
    ...
    def get_stock_history(
        self, symbol: str, start_date, end_date=None, period=None, market=None
    ):
        ...
        try:
            ...
            try:
                stocks = loop.run_until_complete(
                    self.get_industry_stocks(keyword, limit=limit)
                )
                return stocks
            finally:
                loop.close()
...
# 便捷函数
async def get_industry_stocks_from_akshare(
    industry: str,
    impact_type: str = "positive",
    impact_degree: int = 5,
    limit: int = 10,
    ...
    """便捷函数：获取行业股票"""
    return await akshare_industry_provider.get_industry_stocks(
