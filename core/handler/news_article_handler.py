...
from ..models.news_article import NewsArticle, NewsArticleCreate, NewsArticleUpdate, ArticleStatus
...
class NewsArticleHandler:
    ...
    async def create_news_article(self, article_data: Dict[str, Any]) -> Optional[NewsArticle]:
        """
        创建新闻文章
    ...
    async def get_articles_by_source(self, source_id: int, limit: int = 50, offset: int = 0) -> List[NewsArticle]:
        """
        根据新闻源获取文章列表
        ...
        """
        try:
            return await self.repository.get_articles_by_source_id(source_id, limit, offset)
        except Exception as e:
    ...
    async def get_pending_articles(self, limit: int = 100) -> List[NewsArticle]:
        ...
        """
        try:
            articles = await self.repository.get_articles_by_status(ArticleStatus.PENDING, limit)
    ...
    async def update_article(self, article_id: int, update_data: Dict[str, Any]) -> Optional[NewsArticle]:
        """
        更新新闻文章
        ...
        try:
            ...
            # 执行更新
            result = await self.repository.update_news_article(article_id, article_update)
    ...
    async def batch_create_articles(self, articles_data: List[Dict[str, Any]]) -> List[NewsArticle]:
        """
        批量创建新闻文章
        ...
        try:
            ...
            for article_data in articles_data:
                try:
                    # 验证必要字段
                    required_fields = ['title', 'url', 'source_id']
                    if all(field in article_data and article_data[field] for field in required_fields):
                        create_models.append(NewsArticleCreate(**article_data))
                    else:
                        logger.warning(f"跳过无效文章数据: {article_data.get('title', 'Unknown')}")
                except Exception as e:
                    logger.error(f"转换文章数据失败: {e}")
                    continue
            
            # 批量创建
            created_articles = await self.repository.batch_create_articles(create_models)
    ...
    async def update_article_nlp_results(self, article_id: int, nlp_results: Dict[str, Any]) -> bool:
        """
        更新文章的NLP处理结果
        ...
        try:
            ...
            if 'market_relevance_score' in nlp_results:
                update_data['market_relevance_score'] = nlp_results['market_relevance_score']
    ...
    async def get_high_importance_articles(self, min_score: float = 0.7, limit: int = 50) -> List[NewsArticle]:
        """
        获取高重要性的新闻文章
    ...
    async def validate_article_data(self, article_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        验证新闻文章数据
