...
from ..models.news_source import NewsSource, NewsSourceCreate, NewsSourceUpdate, NewsSourceStatus, NewsSourceType
...
class NewsSourceHandler:
    ...
    async def create_news_source(self, source_data: Dict[str, Any]) -> Optional[NewsSource]:
        """
        创建新闻源
    ...
    async def get_active_news_sources(self) -> List[NewsSource]:
        ...
        """
        try:
            return await self.repository.get_news_sources_by_status(NewsSourceStatus.ACTIVE)
        except Exception as e:
            logger.error(f"获取活跃新闻源失败: {e}")
            return []
    
    async def update_news_source(self, source_id: int, update_data: Dict[str, Any]) -> Optional[NewsSource]:
        """
        更新新闻源
    ...
    async def toggle_news_source_status(self, source_id: int) -> Optional[NewsSource]:
        ...
        try:
            ...
            # 切换状态
            new_status = (NewsSourceStatus.INACTIVE 
    ...
    async def update_source_fetch_result(self, source_id: int, success: bool, 
                                       error_message: str = None, article_count: int = 0) -> bool:
        """
        更新新闻源抓取结果
    ...
    async def batch_create_news_sources(self, sources_data: List[Dict[str, Any]]) -> List[NewsSource]:
        """
        批量创建新闻源
    ...
    async def validate_news_source_config(self, source_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        验证新闻源配置
        ...
        try:
            ...
            if 'max_articles_per_fetch' in source_data:
                ...
                if not isinstance(max_articles, int) or max_articles < 1 or max_articles > 200:
