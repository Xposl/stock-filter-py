...
class StrategyEvaluator:
    ...
    def evaluate_strategy(self, strategy_obj, kl_data, name=None, simple_mode=False):
        ...
        backtest_result = self.backtest_engine.run_backtest(
            strategy_obj, kl_data, simple_mode
        )
    ...
    def _calculate_period_performance(self, backtest_result):
        ...
        # 月度绩效
        monthly_returns = (
            equity_df["daily_return"].resample("ME").apply(lambda x: (1 + x).prod() - 1)
        )

        # 季度绩效
        quarterly_returns = (
            equity_df["daily_return"].resample("QE").apply(lambda x: (1 + x).prod() - 1)
        )

        # 年度绩效
        yearly_returns = (
            equity_df["daily_return"].resample("YE").apply(lambda x: (1 + x).prod() - 1)
        )
    ...
    def _calculate_risk_metrics(self, backtest_result):
        """计算风险指标"""
        equity_curve = [
            point["total_value"] for point in backtest_result["equity_curve"]
        ]
        daily_returns = []
        ...
        sharpe_ratio = (
            (np.mean(excess_returns) * 252) / (std_returns * np.sqrt(252))
            if std_returns > 0
            else 0
        )
    ...
    def _calculate_strategy_rating(self, backtest_result, regime_analysis):
        ...
        if "bull" in regime_analysis and "bear" in regime_analysis:
            ...
            # 熊牛市都有不错的胜率
            consistency = (
                min(bull_win_rate, bear_win_rate) / max(bull_win_rate, bear_win_rate)
                if max(bull_win_rate, bear_win_rate) > 0
                else 0
            )
