...
class BacktestVisualizer:
    ...
    def visualize_backtest(self, backtest_result, kl_data, title=None, days=300, show=True, save_path=None):
        """可视化回测结果
        ...
        if days is not None and days < len(df):
            ...
            equity_data = equity_data.iloc[-days:] if len(equity_data) > days else equity_data
        ...
        index_map = {date.strftime('%Y-%m-%d'): idx for idx, date in zip(df['index'], df.index)}
        ...
        # 绘制买入和卖出标记
        if buys_idx:
            ax1.scatter(buys_idx, buys_price, s=100, marker='^', color=buy_colors, zorder=5, label='买入')
            
        if sells_idx:
            ax1.scatter(sells_idx, sells_price, s=100, marker='v', color=sell_colors, zorder=5, label='卖出')
        ...
        for date in equity_data.index:
            ...
            if date_str in index_map:
                ...
                # 如果日期不在索引映射中，找最近的索引
                closest_date = min(index_map.keys(), key=lambda x: abs(pd.to_datetime(x) - date))
        ...
        if len(equity_idx) > 0:
            # 绘制资金曲线
            ax3.plot(equity_idx, equity_data['total_value'].values, label='总资产', color='blue', linewidth=2)
            ax3.plot(equity_idx, equity_data['capital'].values, label='现金', color='green', linewidth=1, alpha=0.7)
            ax3.set_ylabel('资产价值')
            ax3.legend()
            ax3.grid(True)
            
            # 绘制仓位信息
            ax4.plot(equity_idx, equity_data['holdings'].values, label='持仓量', color='purple', linewidth=1.5)
            ax4.set_ylabel('持仓量')
            ax4.grid(True)
        
        # 设置x轴范围和标签
        if start < len(df['index']):
            xlim_min = df['index'].values[0] if start == 0 else df['index'].values[start]
            ...
            ax4.set_xticklabels([date.strftime('%Y-%m-%d') for date in shown_dates], rotation=30)
    ...
    def visualize_performance(self, backtest_result, title=None, show=True, save_path=None):
        """可视化策略性能指标
        ...
        values = [
            ...
            backtest_result['costs']['total_slippage'] + backtest_result['costs']['total_commission']
        ]
        
        ax1.pie(values, labels=labels, autopct='%1.1f%%', startangle=90, colors=['green', 'red', 'gray'])
        ax1.set_title('收益构成分析')
        
        # 2. 交易胜负比例
        labels = ['盈利交易', '亏损交易']
        values = [metrics['summary']['winning_trades'], metrics['summary']['losing_trades']]
        
        ax2.pie(values, labels=labels, autopct='%1.1f%%', startangle=90, colors=['green', 'red'])
        ...
        ax4.axis('tight')
        ax4.axis('off')
        table = ax4.table(cellText=summary_data, loc='center', cellLoc='left', colWidths=[0.3, 0.2])
    ...
    def visualize_trades_analysis(self, backtest_result, title=None, show=True, save_path=None):
        """可视化交易分析
