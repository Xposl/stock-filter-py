...
class NewsSourceRepository:
    ...
    async def get_news_sources_by_status(
        self, status: NewsSourceStatus
    ) -> list[NewsSource]:
        """
        根据状态获取新闻源列表
    ...
    async def update_news_source(
        self, source_id: int, update_data: NewsSourceUpdate
    ) -> Optional[NewsSource]:
        """
        更新新闻源
    ...
    async def update_last_fetch_info(
        self, source_id: int, error_message: str = None, article_count: int = 0
    ) -> bool:
        """
        更新新闻源的最后抓取信息
    ...
    async def get_sources_for_update(self, max_sources: int = 50) -> list[NewsSource]:
        ...
        try:
            ...
            results = self.db.query(
                sql, {"status": NewsSourceStatus.ACTIVE.value, "limit": max_sources}
            )
    ...
    async def get_news_source_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            stats["by_type"] = (
                {row["source_type"]: row["count"] for row in type_results}
                if type_results
                else {}
            )
            ...
            stats["total_articles_fetched"] = (
                total_result["total"] if total_result and total_result["total"] else 0
            )
