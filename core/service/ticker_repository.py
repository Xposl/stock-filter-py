...
from core.models.ticker import (
    Ticker,
    TickerCreate,
    TickerUpdate,
    dict_to_ticker,
...
class TickerRepository:
                ...
                if isinstance(result["update_time"], str):
                    try:
                        return datetime.fromisoformat(
                            result["update_time"].replace("Z", "+00:00")
                        )
                    except ValueError:
                        return datetime.strptime(
                            result["update_time"], "%Y-%m-%d %H:%M:%S"
                        )
                else:
    ...
    def create(
        self, code: str, name: str, data: Union[dict, TickerCreate], commit: bool = True
    ) -> Optional[Ticker]:
        """
        创建新的股票记录
    ...
    def update(
        self, code: str, name: str, data: Union[dict, TickerUpdate], commit: bool = True
    ) -> Optional[Ticker]:
        """
        更新股票记录
    ...
    def update_or_create(
        self, code: str, name: str, data: Union[dict, TickerCreate, TickerUpdate]
    ) -> Optional[Ticker]:
        """
        更新或创建股票记录
